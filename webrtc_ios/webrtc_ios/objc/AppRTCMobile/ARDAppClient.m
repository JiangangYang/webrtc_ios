...
- (void)connectToRoomWithId:(NSString *)roomId
  ...
  [_turnClient requestServersWithCompletionHandler:^(NSArray *turnServers,
                                                     NSError *error) {
    RTCLog(@"turnServers:%@",turnServers);
    if (error) {
  ...
  [_roomServerClient joinRoomWithRoomId:roomId
    ...
    RTCLog(@"Joined room:%@ on room server.", roomId);
          ...
          RTCLog(@"Joined room:%@ on room server.", roomId);
          RTCLog(@"Joined response.roomId:%@", response.roomId);
          RTCLog(@"Joined response.clientId:%@",response.clientId);
          RTCLog(@"Joined response.isInitiator:%d", response.isInitiator);
          RTCLog(@"Joined response.messages:%@",response.messages);
          RTCLog(@"Joined response.webSocketURL:%@", response.webSocketURL);
          RTCLog(@"Joined response.webSocketRestURL:%@", response.webSocketRestURL);
